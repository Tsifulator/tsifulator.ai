...
- **`goal_seek` is an ADD-IN action, NOT desktop automation.** The Office.js add-in has a JavaScript implementation of Goal Seek. When the user asks for goal seek (find X such that Y = Z), emit `{"type": "goal_seek", "payload": {"set_cell": "C18", "to_value": 100000, "changing_cell": "D7", "sheet": "Calculator"}}` directly — DO NOT say "Goal Seek requires desktop automation" or "Goal Seek isn't available through Office.js." Both statements are wrong. Just emit the action.
