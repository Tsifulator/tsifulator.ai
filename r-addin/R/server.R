    ...
    execute_r_action <- function(action, add_message) {
      ...
      if (type == "run_r_code") {
        ...
        tryCatch({
          ...
          # Build the script as a proper R file (no inline escaping nightmare)
          script_lines <- c(
            'local({',
            '  ctx <- tryCatch(rstudioapi::getSourceEditorContext(), error = function(e) NULL)',
            '  if (is.null(ctx) || !nzchar(ctx$id)) { message("[tsifl] No active editor"); return() }',
            '',
            '  # Join all lines into single string for regex replacement',
            '  txt <- paste(ctx$contents, collapse = "\n")',
            '',
            ...
            '    code <- chunks[[ex_name]]',
            '    # Extract exercise number for flexible matching',
            '    ex_num <- gsub("[^0-9]", "", ex_name)',
            '    # Pattern: #### Exercise N ... ```{r...} ... (empty or whitespace) ... ```',
            '    # Capture the chunk header and replace the empty body',
            '    pat <- paste0(',
            '      "(####\\\\s+Exercise\\\\s+", ex_num, ".*?```\\\\{r[^}]*\\\\}\\\\s*\\n)",',
            '      "(\\\\s*\\n)",',
            '      "(```)"',
            '    )',
            '    replacement <- paste0("\\\\1", code, "\\n", "\\\\3")',
            ...
            '  }',
            '',
            '  # Fill text answers (between #### header and ```{r})',
            '  for (ex_name in names(answers)) {',
            '    answer <- answers[[ex_name]]',
            '    ex_num <- gsub("[^0-9]", "", ex_name)',
            '    # Insert answer text after the #### Exercise N line, before ```{r}',
            '    pat <- paste0(',
            '      "(####\\\\s+Exercise\\\\s+", ex_num, "\\\\s*\\n)",',
            '      "(\\\\s*\\n)*(```\\\\{r)"',
            '    )',
            '    replacement <- paste0("\\\\1\\n", answer, "\\n\\n\\\\3")',
            '    txt <- sub(pat, replacement, txt, perl = TRUE)',
            '  }',
            '',
            '  rstudioapi::setDocumentContents(txt, id = ctx$id)',
